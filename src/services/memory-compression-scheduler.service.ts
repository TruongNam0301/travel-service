...
export class MemoryCompressionSchedulerService implements OnModuleInit {
  ...
  constructor(
    ...
    this.compressionEnabled = config?.enabled ?? true;
    this.archiveThreshold = config?.archiveThreshold ?? 1000;
    this.inactivePlanDays = config?.inactivePlanDays ?? 30;
    this.defaultMode = config?.defaultMode ?? "light";
  }
