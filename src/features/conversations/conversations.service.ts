...
export class ConversationsService {
  ...
  > {
    ...
    const sortBy = query.sortBy ?? "lastMessageAt";
    const sortOrder = query.sortOrder ?? "DESC";

    // Explicitly handle page and limit with number type
  ...
  async findOne(conversationId: string, userId: string): Promise<Conversation> {
    const conversation = await this.conversationsRepository.findOne({
      where: { id: conversationId, plan: { userId } },
      relations: ["plan"],
    });

    if (!conversation) {
      throw ResourceException.notFound("Conversation", conversationId);
    }

    // Verify plan exists and check ownership
