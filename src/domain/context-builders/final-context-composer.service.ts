...
import { CONTEXT_BUILDER_MIN_TOKENS } from "../../core/constants/context-builder.constant";
...
export class FinalContextComposer {
  ...
  constructor(
    ...
    @Inject(contextBuilderConfig.KEY)
    private readonly contextBuilderCfg: ConfigType<typeof contextBuilderConfig>,
  ) {}
  ...
  async composeContext(options: FinalContextOptions): Promise<FinalContext> {
    const {
      ...
      conversationId,
      query,
      maxTokens = this.contextBuilderCfg.maxTokens,
      priorities,
